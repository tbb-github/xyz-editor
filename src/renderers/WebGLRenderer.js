...
import { ShadowMapPlugin } from './webgl/plugins/ShadowMapPlugin.js';
import { SpritePlugin } from './webgl/plugins/SpritePlugin.js';
import { LensFlarePlugin } from './webgl/plugins/LensFlarePlugin.js';
...
function WebGLRenderer( parameters ) {
	...
	this.shadowMapType = 1;//PCFShadowMap
	this.shadowMapCullFace = 2;//CullFaceFront
	...
	this.renderBufferDirect = function ( camera, lights, fog, material, geometry, object ) {
		...
		// 	var mode = ( object.mode === LineStrip ) ? _gl.LINE_STRIP : _gl.LINES;
...
;
	...
	function initMaterial( material, lights, fog, object ) {
		...
		var parameters = {
			...
			doubleSided: material.side === 2,//DoubleSide = 2;
			flipSided: material.side === 1//BackSide = 1;
	...
	function setProgram( camera, lights, fog, material, object ) {
		...
		if ( refreshMaterial ) {
			...
			} else if ( material instanceof PointCloudMaterial ) {
	...
	function refreshUniformsShadow ( uniforms, lights ) {
		...
		if ( uniforms.shadowMatrix ) {
			...
			for ( var i = 0, il = lights.length; i < il; i ++ ) {
				...
				if ( light instanceof SpotLight || ( light instanceof DirectionalLight && ! light.shadowCascade ) ) {
	...
	this.setBlending = function ( blending, blendEquation, blendSrc, blendDst ) {
		...
		if ( blending !== _oldBlending ) {
			...
			} else if ( blending === 5 ) {// CustomBlending 5
		...
		if ( blending === 5 ) {// CustomBlending 5
	...
	function setTextureParameters ( textureType, texture, isImagePowerOfTwo ) {
		...
		if ( isImagePowerOfTwo ) {
			...
			if ( texture.wrapS !== ClampToEdgeWrapping || texture.wrapT !== ClampToEdgeWrapping ) {
				...
				console.warn( 'WebGLRenderer: Texture is not power of two. Texture.wrapS and Texture.wrapT is set to ClampToEdgeWrapping. ( ' + texture.sourceFile + ' )' );
			...
			if ( texture.minFilter !== NearestFilter && texture.minFilter !== LinearFilter ) {
				...
				console.warn( 'WebGLRenderer: Texture is not power of two. Texture.minFilter is set to LinearFilter or NearestFilter. ( ' + texture.sourceFile + ' )' );
		...
		if ( extension && texture.type !== FloatType ) {
	...
	this.uploadTexture = function ( texture ) {
		...
		isImagePowerOfTwo = Math.isPowerOfTwo( image.width ) && Math.isPowerOfTwo( image.height ),
	...
	function clampToMaxSize ( image, maxSize ) {
		...
		if ( image.width > maxSize || image.height > maxSize ) {
			...
			console.log( 'WebGLRenderer:', image, 'is too big (' + image.width + 'x' + image.height + '). Resized to ' + canvas.width + 'x' + canvas.height + '.' );
	...
	this.setRenderTarget = function ( renderTarget ) {
		...
		if ( renderTarget && renderTarget.__webglFramebuffer === undefined ) {
			...
			var isTargetPowerOfTwo = Math.isPowerOfTwo( renderTarget.width ) && Math.isPowerOfTwo( renderTarget.height ),
	...
	function allocateShadows( lights ) {
		...
		for ( var l = 0, ll = lights.length; l < ll; l ++ ) {
			...
			if ( light instanceof DirectionalLight && ! light.shadowCascade ) maxShadows ++;
